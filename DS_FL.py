...
def print_matrix(args, train_dataset, user_groups, fl_local_model_list, device, epoch, test_dataset, data_dom):
    ...
    # 画图
    save_folder = 'D:/python object/federate learning/FedVC_eeg/result_DS-FL/SEED/f{}'.format(args.alpha_d)
    # 确保保存文件夹存在
...
def write_acc(round, acc, args, loss):
    ...
    target_file_path = 'D:/python object/federate learning/FedVC_eeg/result_DS-FL/SEED/f{}'.format(
        args.alpha_d)
    ...
    for epoch in range(args.epochs):
        ...
        if args.loc_epochs != 0:
            for idx in range(args.num_clint):
                ...
                w, loss = local_model.update_weights(model=copy.deepcopy(FL_local_model_list[idx]), num_clint=idx, global_round=epoch)  # w = model.state_dict()
        ...
        client_logits = {}  # 一个字典，里面每个元素是一个client的10个平均logit []
        for client_idx in range(args.num_clint):
            client_logit_avg = []

            # get data  trainloader = DataLoader(dataset[idx], batch_size=self.args.batch_size, shuffle=True)
            private_data = DataLoader(train_dataset[client_idx], batch_size=args.batch_size, shuffle=True)

            # get model
            model = FL_local_model_list[client_idx].to(device)
            model.eval()
            ...
            # prediction
            for batch_idx, (images, labels) in enumerate(private_data):
                images, labels = images.to(device), labels.to(device)
                # prediction
                with torch.no_grad():
                    if data_dom[client_idx] == '2d':
                        images = images.reshape(images.size(0), -1)
                    else:
                        images = images.reshape(images.size(0), 1, -1)

                    _, _, batch_logit = model(images, t=4.0)

                # 将输出的logits分配到相应标签的列表中0
                for logit, label in zip(batch_logit, labels):
                    label_logits[label.item()].append(logit)

            for i in range(args.num_classes):
                if i in label_logits:
                    continue
                else:
                    label_logits[i].append(torch.zeros(args.num_classes, device=device))

            # 计算每个标签的logits平均值
            for label in label_logits:
                # 在这个标签内的所有logits上计算均值
                mean_logits = torch.mean(torch.stack(label_logits[label]), dim=0)
                client_logit_avg.append(mean_logits)

            client_logits[client_idx] = client_logit_avg

        # ---------------upload & aggregation-------------
        Aggregated_logit = []
        for class_idx in range(args.num_classes):
            class_logit = []
            for client in range(args.num_clint):
                class_logit.append(client_logits[client][class_idx])  # class_logit里面是10个client的第class_idx类的logit
            mean_logit = torch.mean(torch.stack(class_logit), dim=0)
            Aggregated_logit.append(mean_logit)
        ...
        for cli_idx in range(args.num_clint):
            # get data
            private_data = DataLoader(train_dataset[cli_idx], batch_size=args.batch_size, shuffle=True)
            ...
            for kd_epoch in range(args.KD_epochs):
                ...
                for batch_idx, (images, labels) in enumerate(private_data):
                    ...
                    # -----------使用Aggregated_logit按照label创建一个soft_logit-----------
                    for label in labels:
                        soft_logit_list.append(Aggregated_logit[label.item()])
                    soft_logit_tensor = torch.stack(soft_logit_list, dim=0)

                    # get client_pre
                    model.train()
                    _, batch_client_soft_pre, batch_client_output = model(images, t=1.0)

                    # criterion
                    loss = criterion(batch_client_output, soft_logit_tensor, labels)
                    # loss = criterion(output, labels)
                    loss.backward()
                    optimizer.step()
                ...
                print('| Client Num : {} | Client Epoch : {} | \tLoss: {:.6f}'.format(
                    cli_idx, kd_epoch, kd_epoch_loss / len(private_data)))
