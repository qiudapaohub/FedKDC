...
def main():
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        # 2. 更新每个client的model
        clients = range(args.num_clint)
        select_clients = random.sample(clients, args.m)
        for cli_idx in select_clients:
            # get model
            ...
            for kd_epoch in range(args.KD_epochs):
                ...
                for batch_idx, (images, labels) in enumerate(proxy_dataset):
                    batch_Pre_local = []  # 所有leader在一个bs上的logit
                    images, labels = images.to(device), labels.to(device)
                    model.zero_grad()
                    optimizer.zero_grad()

                    # -----------sever端聚合所有的soft_output-----------
                    ...
                    with torch.no_grad():
                        for idx in range(args.num_clint):
                            ...
                            _, batch_soft_pre, batch_logit = FL_local_model_list[idx](images, t=4.0)
                            # local_model.prediction(model=FL_local_model_list[idx])
                            batch_Pre_local.append(batch_logit)

                    # 对每个client的pre做均值
                    stacked_tensor = torch.stack(batch_Pre_local, dim=0)
                    Pre_avg = torch.mean(stacked_tensor, dim=0)
                    ...
                    if data_dom[cli_idx] == '2d':
                        ...
                        images = images.reshape(images.size(0), 1, -1)
                    _, batch_client_soft_pre, batch_client_output = model(images, t=4.0)

                    # criterion
                    loss = criterion(batch_client_output, Pre_avg, labels)
                    # loss = criterion(output, labels)
                    loss.backward()
                    optimizer.step()
                ...
                print('| Client Num : {} | Client Epoch : {} | \tLoss: {:.6f}'.format(
                    cli_idx, kd_epoch, kd_epoch_loss / len(proxy_dataset)))
