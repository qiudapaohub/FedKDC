...
def print_matrix(args, train_dataset, user_groups, fl_local_model_list, device, epoch, test_dataset, data_dom):
    ...
    # 画图
    save_folder = 'D:/python object/federate learning/FedVC_eeg/result_FedProx/SEEDIV/f{}'.format(args.alpha_d)
    # 确保保存文件夹存在
...
def write_acc(round, acc, args, loss):
    ...
    target_file_path = 'D:/python object/federate learning/FedVC_eeg/result_FedProx/SEEDIV/f{}'.format(
        args.alpha_d)
...
def main():
    ...
    # --------model preparation--------
    global_model = get_network(args.model11, args).to(device)  #VGG11
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        for idx in idxs_users:
            ...
            w, loss = local_model.update_weights_prox(model=copy.deepcopy(global_model), global_model=copy.deepcopy(global_model),
        ...
        test_acc, outputs_clint, loss_client, conf_matrix = test_inference(args, global_model, test_dataset, device,
                                                                           data_dom[0])
